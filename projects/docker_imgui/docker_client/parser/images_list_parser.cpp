...
#include "images_list_parser.hpp"
#include "parser_utils.hpp"

using namespace docker;
std::vector<ImageListModel> parser::parseImagesList(const std::string src)
{
    rapidjson::Document document;
    document.Parse(src.c_str());

    std::vector<ImageListModel> results;

    if (document.IsArray())
    {
        for (auto const &record : document.GetArray())
        {
            ImageListModel result{};
